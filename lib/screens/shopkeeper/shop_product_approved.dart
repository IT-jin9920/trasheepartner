...
class ShopApprovedProductScreen extends StatefulWidget {
  const ShopApprovedProductScreen({super.key});

  @override
  _ShopApprovedProductScreenState createState() =>
      _ShopApprovedProductScreenState();
}

class _ShopApprovedProductScreenState
    extends State<ShopApprovedProductScreen> {
  List<Map<String, dynamic>> shopList = [];
  int shopId = 0;
  bool isLoading = true; // Loading state

  static const String apiUrl =
      "https://syntaxium.in/DUSTBIN_API/shop_product_approved.php";
  ...
  Future<void> _fetchShopList() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && !result['error']) {
        setState(() {
          shopList = List<Map<String, dynamic>>.from(result['approved_product_details']);
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Approved Product'),
      ),
      body: isLoading
